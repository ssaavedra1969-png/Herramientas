  ...
  grid.innerHTML = vehicles.map(v => {
    ...
    const fotoHtml = hasPhoto
      ? `<div class="relative w-full h-40">
           <img src="${mv.fotoURL}" alt="${mv.patente}" class="w-full h-40 object-cover" onerror="handlePhotoError(this, '${brandLogo}')">
         </div>`
      : fallbackBg;
      ...
      <div class="vehicle-card fade-row" onclick="rowClick('${v.id}', event)">
        ...
        <div class="vehicle-card-footer">
          ...
          <div class="card-actions" onclick="event.stopPropagation()">
            <button onclick="checkTituloFromCard('${(mv.patente || '').replace(/'/g, "\\'")}')" title="Título de propiedad" class="card-titulo-btn hidden">
...
function checkTituloFromCard(patente) {
  if (!patente) return showToast('Vehículo sin patente', 'error');
  fetch('/titulos/' + encodeURIComponent(patente) + '.pdf', { method: 'HEAD' })
    .then(r => {
      if (r.ok) window.open('/titulos/' + encodeURIComponent(patente) + '.pdf', '_blank');
      else showToast('No hay título disponible para ' + patente, 'info');
    })
    .catch(() => showToast('Error al verificar título', 'error'));
}

function checkAllTitulos() {
  document.querySelectorAll('.card-titulo-btn').forEach(btn => {
    const patente = btn.getAttribute('onclick').match(/'([^']+)'/)?.[1];
    if (!patente) return;
    fetch('/titulos/' + encodeURIComponent(patente) + '.pdf', { method: 'HEAD' })
      .then(r => { if (r.ok) btn.classList.remove('hidden'); });
  });
}
